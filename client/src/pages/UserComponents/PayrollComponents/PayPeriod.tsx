...
const PayPeriodComponent: React.FC<PayPeriodProps> = ({
  ...
  const getNextPayPeriod = (date: Date): string => {
    const month = date.toLocaleString("default", { month: "long" });
    const year = date.getFullYear();
    const day = date.getDate();

    if (day >= 1 && day <= 15) {
      // If current period is 1-15, return 16-end of month
      const lastDay = new Date(year, date.getMonth() + 1, 0).getDate();
      return `${month} 16-${lastDay}, ${year}`;
    } else {
      // If current period is 16-end, return 1-15 of next month
  ...
  const generatePayPeriods = () => {
    ...
    // Set the next pay period as default
  ...
  useEffect(() => {
    ...
    // Set up an interval to check and update pay period daily
