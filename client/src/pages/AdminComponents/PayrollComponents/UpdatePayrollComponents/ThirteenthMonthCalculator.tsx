...
interface ThirteenthMonthCalculatorProps {
...
const ThirteenthMonthCalculator = ({
  onCalculate,
  employeeData,
}: ThirteenthMonthCalculatorProps) => {
  ...
  // Set selected employee if employeeData is provided
  useEffect(() => {
    if (employeeData) {
  ...
  const calculateThirteenthMonth = async () => {
    ...
    try {
      ...
      setCalculation({
        totalBasicSalary,
        thirteenthMonthPay,
        monthsWorked: payrollRecordsCount / 2, // Assuming 2 pay periods per month
      });
  ...
  // Generate year options (5 years in the past, current year, and 5 years in the future)
