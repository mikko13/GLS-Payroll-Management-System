...
interface PasswordStrength {
  score: number; // 0-4 (0: too weak, 4: very strong)
  label: string;
  color: string;
}
...
const UpdateUserForm: React.FC<UpdateUserFormProps> = ({
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (user) {
        // Use the user data from location state
        setFormData({
      ...
      // If user data is not in location state, fetch it
      if (id) {
        setIsLoading(true);
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.profilePicture) {
        ...
        // Add a flag to indicate that the current image should be removed
