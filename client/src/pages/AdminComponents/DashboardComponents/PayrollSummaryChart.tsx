...
const PayrollSummaryChart = () => {
    ...
    const fetchPayrollData = async () => {
      try {
        ...
        const periods = [...new Set(data.map((item) => item.payPeriod))].sort(
          (a, b) => {
            // Sort by date (assuming format like "Month 1-15, Year" or "Month 16-31, Year")
            ...
            // Then compare date ranges (1-15 comes before 16-31)
  ...
  const processPayrollData = (data, timeRange, periods) => {
    if (!data || data.length === 0) {
      return [];
    }

    // Determine how many periods to include based on timeframe
    ...
    relevantPeriods.forEach((period) => {
      periodData[period] = {
        ...
        totalHoliday: 0, // Combines regular and special holiday pay
        totalOvertime: 0,
        total13thMonth: 0,
        totalDeductions: 0,
        totalNetPay: 0,
    ...
    return Object.values(periodData).map((item) => {
      // Calculate average net pay per employee
  ...
  // Helper function to display shortened period labels
